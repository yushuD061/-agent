...
from privacy import safe_print, safe_print_exception
...
class QQChannel(Channel):
    ...
    async def send(self, message: OutboundMessage) -> None:
        ...
        if self._bot is None or not hasattr(self._bot, 'api'):
            raise RuntimeError("qq_bot_not_ready")

        try:
            if message.target_type == "group":
                await self._bot.api.post_group_message(
                    group_openid=message.chat_id, msg_type=0, msg_id="", content=message.content)
                print(f"[QQChannel] 群消息已发送到 {message.chat_id}")
                return

            if message.target_type == "c2c":
                await self._bot.api.post_c2c_message(
                    openid=message.chat_id, msg_type=0, msg_id="", content=message.content)
                print(f"[QQChannel] C2C消息已发送到 {message.chat_id}")
                return

            # 普通聊天回复保持兼容：先尝试 C2C，再尝试群聊。
