...
class MCPClientManager:
    ...
    def __init__(self, mcp_config: dict):
        ...
        self._connection_order = []  # 严格按 LIFO 关闭 AnyIO cancel scopes
    ...
    async def _connect_one(self, server_name: str, server_config: dict) -> None:
        ...
        self._connection_order.append(server_name)
        ...
        for server_name, server_config in self.mcp_config.items():
            try:
                async with asyncio.timeout(timeout):
                    await self._connect_one(server_name, server_config)
    ...
    async def shutdown(self) -> None:
        ...
        # 每个 stdio/session 是嵌套 cancel scope，必须严格后进先出成对退出。
        for server_name in reversed(self._connection_order):
            session = self._session_managers.get(server_name)
            try:
                if session is not None:
                    await session.__aexit__(None, None, None)
            except Exception as e:
                print(f"[!] 关闭 session {server_name} 失败: {e}")
            ctx = self._context_managers.get(server_name)
            try:
                if ctx is not None:
