...
from privacy import sanitize_data, safe_print as print
...
class SessionManager:
    ...
    def _get_session_path(self, session_key: str) -> str:
        ...
        Returns:
            str: 会话文件的完整路径
        """
        # M1 conversation metadata can point at an existing legacy Web file.
        parts = session_key.split(":")
        if len(parts) == 3 and parts[:2] == ["web", "local"]:
            index_path = os.path.join(self.sessions_dir, "conversations.json")
            try:
                with open(index_path, "r", encoding="utf-8") as handle:
                    for item in json.load(handle).get("conversations", []):
                        if item.get("conversation_id") == parts[2]:
                            filename = item.get("message_file", "")
                            if os.path.basename(filename) == filename and filename.endswith(".jsonl"):
    ...
    def save_message(self, session_key: str, message: dict[str, Any]) -> None:
        ...
        # 添加时间戳
        message_with_timestamp = sanitize_data(message.copy())
